...
pbxProject.prototype.getBuildProperty = function(prop, build) {
    var target = this.getFirstTarget().firstTarget;
    var configs = this.pbxXCConfigurationList()[target.buildConfigurationList];
    var buildConfigName = build || configs.defaultConfigurationName;

    for (var i = 0; i < configs.buildConfigurations.length; i += 1) {
        var config = configs.buildConfigurations[i];
        var section = this.pbxXCBuildConfigurationSection()[config.value];
        if (section.name === buildConfigName) {
            return section.buildSettings[prop];
        }
    }
